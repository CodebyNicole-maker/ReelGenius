...
class favoriteMovieService {
  getmoviesArray() {
    const favMovies: string[] |number[] | null = auth.getProfile().favorite_movies; // Get the logged-in user's ID
  ...
  getSingleMovie(index: number): number | null {
    ...
    async addFavoriteMovie(movieID: number) {
      try {
    ...
    async removeFavoriteMovie(movieID: number) {
      try {
      const updatedFavorites = this.getmoviesArray()?.filter(movie => movie !== movieID);
      const response = await fetch('/api/favorites', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
